...
class VideoGenerationFlow(BaseFlow):
    ...
    def execute(self, input_data: Dict[str, Any]) -> FlowResult:
        ...
        try:
            ...
            if not self.validate_input(
                input_data, ["question", "formatted_explanation"]
            ):
                return FlowResult(
                    success=False,
                    data=input_data,
            ...
            if not video_path:
                return FlowResult(
                    success=False, data=input_data, error="Video generation failed"
                )
    ...
    def _create_slides(
        self, question_data: Dict[str, Any], explanation: str
    ) -> List[Path]:
        """Create slide images."""
        slides = []
    ...
    def _create_question_slide(self, question_data: Dict[str, Any]) -> Path:
        ...
        slide_path = (
            self.temp_dir / f"question_slide_{datetime.now().strftime('%H%M%S')}.png"
        )
    ...
    def _create_options_slide(self, question_data: Dict[str, Any]) -> Path:
        ...
        slide_path = (
            self.temp_dir / f"options_slide_{datetime.now().strftime('%H%M%S')}.png"
        )
        ...
        for i, option in enumerate(question_data["options"]):
            ...
            color = (
                "green" if chr(65 + i) == question_data["correct_answer"] else "black"
            )
    ...
    def _create_explanation_slide(self, explanation: str) -> Path:
        ...
        slide_path = (
            self.temp_dir / f"explanation_slide_{datetime.now().strftime('%H%M%S')}.png"
        )
        ...
        if "\n" in clean_explanation and (
            "Step" in clean_explanation or "Equation" in clean_explanation
        ):
            ...
            for line in clean_explanation.split("\n"):
                if line.strip():
                    wrapped = self._wrap_text(
                        line, 80
                    )  # Slightly less width for equations
    ...
    def _clean_explanation_text(self, explanation: str) -> str:
        ...
        if "Step" in explanation and (
            "Equation" in explanation or "Result" in explanation
        ):
            ...
            for line in explanation.split("\n"):
                ...
                if line:
                    ...
                    clean_line = clean_line.replace(
                        "*This solution was generated", "Generated"
                    )
    ...
    def _generate_audio(self, explanation: str) -> Path:
        """Generate audio narration."""
        audio_path = (
            self.temp_dir / f"narration_{datetime.now().strftime('%H%M%S')}.mp3"
        )
    ...
    def _combine_slides_and_audio(
        self, slides: List[Path], audio_path: Path, output_path: Path
    ):
        ...
        try:
            ...
            for slide_path in slides:
                ...
                clip = (
                    VideoFileClip(str(slide_path), duration=slide_duration)
