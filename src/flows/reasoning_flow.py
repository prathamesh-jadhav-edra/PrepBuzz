...
class ReasoningExtractionFlow(BaseFlow):
    ...
    def _search_for_reasoning(self, question_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            search_results = google_searcher.search_cat_question_context(question_text, subject)
            
            if search_results:
                reasoning_text = self._extract_reasoning_from_web_results(search_results)
                if reasoning_text and self._is_useful_reasoning(reasoning_text):
                    self.logger.info(f"Found useful reasoning context from {len(search_results)} sources")
                    return reasoning_text
                else:
                    self.logger.info("Search results found but content not useful, using fallback reasoning")
    ...
    def _extract_reasoning_from_web_results(self, search_results: List[Dict[str, str]]) -> str:
        ...
        for result in search_results[:3]:  # Process top 3 results
            ...
            if any(keyword in title.lower() for keyword in ["solution", "explanation", "approach", "solve", "cat", "exam"]):
    ...
    def _is_useful_reasoning(self, reasoning_text: str) -> bool:
        ...
        useless_count = sum(1 for phrase in useless_phrases if phrase in reasoning_lower)
        ...
        useful_indicators = [
            "answer", "because", "correct", "option", "explanation", "reason",
            "solve", "approach", "method", "analysis", "conclusion"
        ]
        
        useful_count = sum(1 for indicator in useful_indicators if indicator in reasoning_lower)
